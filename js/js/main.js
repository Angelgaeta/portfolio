  ...
  function quickQuoteFormSetup() {
    ...
    function toast(msg, ok = false) {
      if ($errors.length) {
        $errors
          .html(
            `<div class="alert ${ok ? "alert-success" : "alert-danger"}" style="margin:0;">${msg}</div>`,
          )
          .show();
    ...
    function setStepLock(stepNumber, locked) {
      ...
      if (stepNumber === 3) {
        $step
          .find("#qq-w-next, #qq-w-back, #qq-w-generate")
    ...
    function updateLocksAndProgress() {
      ...
      setStatus(
        $step3Status,
        okEmail && okType ? "À compléter" : "Verrouillé",
        okEmail && okType,
      );
    ...
    const wizard = {
      vitrine: {
        ...
        steps: [
          ...
          {
            q: "Style souhaité ?",
            opts: ["Sobre / pro", "Créatif", "Premium", "Je ne sais pas"],
          },
          {
            q: "Fonctions utiles ?",
            opts: ["Formulaire", "SEO", "Blog", "Portfolio/Galerie"],
            multi: true,
          },
        ],
      },
      app: {
        ...
        steps: [
          {
            q: "Problème à résoudre ?",
            opts: ["Gagner du temps", "Automatiser", "Centraliser"],
          },
          {
            q: "Qui l’utilise ?",
            opts: ["Moi", "Mon équipe", "Clients", "Public"],
          },
          {
            q: "Fonctionnalités ?",
            opts: [
      ...
      ux: {
        ...
        steps: [
          {
            q: "Point de départ ?",
            opts: ["Idée", "Site existant", "Maquettes déjà faites"],
          },
          {
            q: "Objectif UX ?",
            opts: ["Clarté", "Conversion", "Préparer le dev", "Refonte"],
          },
          {
            q: "Livrables ?",
            opts: ["Wireframes", "Maquettes", "Prototype", "Design system"],
            multi: true,
          },
        ],
      },
      refonte: {
        ...
        steps: [
          {
            q: "Qu’est-ce qui pose problème aujourd’hui ?",
            opts: [
              "Design daté",
              "Trop lent",
              "Pas de demandes",
          ...
          {
            q: "Qu’est-ce qu’on garde ?",
            opts: ["Logo/charte", "Textes", "Structure", "Je ne sais pas"],
            multi: true,
          },
          {
            q: "Priorité refonte ?",
            opts: ["Design", "SEO", "Conversion", "Performance"],
            multi: true,
          },
        ],
      },
    ...
    function renderWizardStep() {
      ...
      step.opts.forEach((o) => {
        const $b = $(
          '<button type="button" class="qq-chip" aria-pressed="false"></button>',
        ).text(o);
    ...
    $form.on("click", ".qq-chip[data-fill]", function () {
      ...
      if (!isStep3Unlocked()) {
        ...
        const target = !isValidEmail($email.val())
          ? $email.get(0)
          : $type.get(0);
      ...
      if (!$helper.length || !$opts.length || !$next.length || !$gen.length) {
        ...
        toast(
          "Le configurateur n’est pas disponible (structure HTML manquante).",
        );
        return;
      }
      ...
      if (!wizard[fill]) {
        ...
        toast(
          "Cette option n’est pas encore disponible 🙂 Choisis une autre option.",
        );
        return;
      }
    ...
    $form.find(".qq-chip[data-fill]").each(function () {
      ...
      if (!wizard[fill]) {
        $(this)
          ...
          .attr("title", "Option bientôt disponible");
    ...
    $form.on("submit", function (e) {
      ...
      if (!isValidEmail($email.val()))
        errs.push("Merci d’indiquer un email valide.");
      if (!$type.val()) errs.push("Merci de choisir un type de projet.");
      if (($msg.val() || "").trim().length < 10)
        errs.push("Décris ton besoin en au moins 10 caractères.");
      ...
      $.ajax({
        ...
        error: function () {
          showErrors([
            "Erreur lors de l’envoi. Tu peux me contacter directement par email.",
          ]);
        },
      });
    });
